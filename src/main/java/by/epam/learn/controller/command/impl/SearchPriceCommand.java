...
/**
 * The {@code SearchPriceCommand} class represents search price
 * 
 * @author Ihar Klepcha
 */
public class SearchPriceCommand implements Command {
	public static Logger log = LogManager.getLogger();
	private final PriceServiceImpl service;

	public SearchPriceCommand(PriceServiceImpl service) {
	...
	public Router execute(HttpServletRequest request) {
		Router router;
		String operationValue = request.getParameter(RequestParameter.OPERATION);
		try {
			List<Price> prices = service.findPriceByOperation(operationValue);
			if (!prices.isEmpty()) {
